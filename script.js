...
function getStoreStock(productCode) {
    // Special case: treat MI and ML as equivalent
    if (typeof productCode === 'string' && (productCode.startsWith('MI') || productCode.startsWith('ML'))) {
        const codeSuffix = productCode.slice(2);
        const miCode = 'MI' + codeSuffix;
        const mlCode = 'ML' + codeSuffix;
        let total = 0;
        // Sum from mrpData (storeStock field)
        if (mrpData.hasOwnProperty(miCode) && mrpData[miCode].storeStock !== undefined) {
            total += parseNumericValue(mrpData[miCode].storeStock);
        }
        if (mrpData.hasOwnProperty(mlCode) && mrpData[mlCode].storeStock !== undefined) {
            total += parseNumericValue(mrpData[mlCode].storeStock);
        }
        // Sum from inventoryData (value is number)
        if (inventoryData.hasOwnProperty(miCode)) {
            total += parseNumericValue(inventoryData[miCode]);
        }
        if (inventoryData.hasOwnProperty(mlCode)) {
            total += parseNumericValue(inventoryData[mlCode]);
        }
        return total;
    }
    // Fallback: normal lookup
    if (mrpData.hasOwnProperty(productCode) && mrpData[productCode].storeStock !== undefined) {
        return parseNumericValue(mrpData[productCode].storeStock);
    }
    if (inventoryData.hasOwnProperty(productCode)) {
        return parseNumericValue(inventoryData[productCode]);
    }
